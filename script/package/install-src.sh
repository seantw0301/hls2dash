...
#   PREFIX=/opt/hls2dash sudo ./install.sh
#   PREFIX=$HOME/hls2dash ./install.sh
set -euo pipefail
...
if [[ "$PREFIX" != "$PKG_ROOT" ]] && [[ "$(id -u)" -ne 0 ]] && [[ "$PREFIX" == /opt/* || "$PREFIX" == /usr/* ]]; then
  ...
  echo "  PREFIX=\$HOME/hls2dash ./install.sh"
...
echo "=== hls2dash install (from source) ==="
...
BIN_SRC="${TARGET_DIR}/release/hls2dash"
if [[ ! -x "$BIN_SRC" ]]; then
...
install -m 755 "$BIN_SRC" "${PREFIX}/bin/hls2dash"
...
echo "  binary: ${PREFIX}/bin/hls2dash"
echo "  start:  ${PREFIX}/start.sh"
echo "  stop:   ${PREFIX}/stop.sh"
echo "  config: ${PREFIX}/config.yaml"
echo
echo "Play (dash):   http://<host>:<port>/live/<channel>/index.mpd"
echo "Play (mpegts): http://<host>:<port>/live/<channel>/mpegts"
