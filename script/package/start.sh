#!/usr/bin/env bash
# 一鍵啟動：背景執行 hls2dash（binary dist / install.sh 產出的 ./bin）
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$ROOT"

PID_FILE="${ROOT}/logs/hls2dash.pid"
LOG_FILE="${ROOT}/logs/hls2dash.log"
CONFIG="${CONFIG:-${ROOT}/config.yaml}"
BIN="${ROOT}/bin/hls2dash"
...
echo "Starting hls2dash (config=${CONFIG})..."
...
MODE="$(awk '/^output_mode:/{print $2;exit}' "$CONFIG" 2>/dev/null || echo dash)"
MODE="${MODE//\"/}"
echo "hls2dash started (pid=${pid})"
echo "  log: $LOG_FILE"
if [[ "$MODE" == "mpegts" ]]; then
  echo "  play: http://<host>:${DASH_PORT}/live/<channel>/mpegts"
else
